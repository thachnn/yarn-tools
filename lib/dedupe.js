...
function collectPackages(obj, options) {
  ...
  var result = {},
    /** @type {Object.<string, Object.<string, VersionInfo>>} */
    pkgVersions = {};

  for (var key in obj) {
    /** @type {Array.<string|PackageInfo>} */
    var arr = utils.splitPackageKey(key),
      name = arr[0];
    ...
    var pkg = obj[key],
      ver = pkg.version,
      requestedVersion = arr[1] || '*', // no version specified
...
function computeBestVersions(instances, options) {
  ...
  for (var p of instances) {
    ...
    var candidateVersions = Array.from(p.satisfiedBy);
    candidateVersions.sort((v1, v2) => {
      // sort versions based on how many packages it satisfies
      if (opts.useMostCommon) {
        var size1 = versions[v1].satisfies.size,
          size2 = versions[v2].satisfies.size;
    ...
    p.bestVersion = candidateVersions[0];
  }
