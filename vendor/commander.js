var EventEmitter = require("events").EventEmitter, spawn = require("child_process").spawn, path = require("path"), dirname = path.dirname, basename = path.basename, fs = require("fs");

function Option(flags, description) {
  this.flags = flags, this.required = flags.indexOf("<") >= 0, this.optional = flags.indexOf("[") >= 0, 
  this.mandatory = !1, this.negate = -1 !== flags.indexOf("-no-"), (flags = flags.split(/[ ,|]+/)).length > 1 && !/^[[<]/.test(flags[1]) && (this.short = flags.shift()), 
  this.long = flags.shift(), this.description = description || "";
}

require("util").inherits(Command, EventEmitter), exports = module.exports = new Command, 
exports.Command = Command, exports.Option = Option, Option.prototype.name = function() {
...
class CommanderError extends Error {
  constructor(exitCode, code, message) {
    ...
    this.code = code, this.exitCode = exitCode;
  }
}

function Command(name) {
  this.commands = [], this.options = [], this._execs = new Set, this._allowUnknownOption = !1, 
  this._args = [], this._name = name || "", this._optionValues = {}, this._storeOptionsAsProperties = !0, 
  this._passCommandToAction = !0, this._actionResults = [], this._helpFlags = "-h, --help", 
  this._helpDescription = "output usage information", this._helpShortFlag = "-h", 
...
function wrap(str, width, indent) {
  ...
  return (str.match(regex) || []).map((function(line, i) {
    return "\n" === line.slice(-1) && (line = line.slice(0, line.length - 1)), (i > 0 && indent ? Array(indent + 1).join(" ") : "") + line.trimRight();
  })).join("\n");
}
...
function outputHelpIfRequested(cmd, options) {
  options = options || [];
  for (var i = 0; i < options.length; i++) options[i] !== cmd._helpLongFlag && options[i] !== cmd._helpShortFlag || (cmd.outputHelp(), 
...
function incrementNodeInspectorPort(args) {
  return args.map((arg => {
    ...
    if (0 === arg.indexOf("--inspect")) {
      var debugOption, match, debugHost = "127.0.0.1", debugPort = "9229";
...
exports.CommanderError = CommanderError, Command.prototype.command = function(nameAndArgs, actionOptsOrExecDesc, execOpts) {
  var desc = actionOptsOrExecDesc, opts = execOpts;
  "object" == typeof desc && null !== desc && (opts = desc, desc = null), opts = opts || {};
  var args = nameAndArgs.split(/ +/), cmd = new Command(args.shift());
  return desc && (cmd.description(desc), this.executables = !0, this._execs.add(cmd._name), 
  opts.isDefault && (this.defaultExecutable = cmd._name)), cmd._noHelp = !!opts.noHelp, 
  cmd._helpFlags = this._helpFlags, cmd._helpDescription = this._helpDescription, 
  ...
  cmd._passCommandToAction = this._passCommandToAction, cmd._executableFile = opts.executableFile, 
  ...
  return this._exitCallback = fn || function(err) {
  ...
  var self = this, listener = function(args, unknown) {
    ...
    outputHelpIfRequested(self, parsed.unknown), self._checkForMissingMandatoryOptions(), 
    parsed.unknown.length > 0 && self.unknownOption(parsed.unknown[0]), parsed.args.length && (args = parsed.args.concat(args)), 
    ...
    var expectedArgsCount = self._args.length, actionArgs = args.slice(0, expectedArgsCount);
    self._passCommandToAction ? actionArgs[expectedArgsCount] = self : actionArgs[expectedArgsCount] = self.opts(), 
  ...
  if (option.negate || option.optional || option.required || "boolean" == typeof defaultValue) {
    ...
    void 0 !== defaultValue && (self._setOptionValue(name, defaultValue), option.defaultValue = defaultValue);
  }
  return this.options.push(option), this.on("option:" + oname, (function(val) {
    null !== val && fn && (val = fn(val, void 0 === self._getOptionValue(name) ? defaultValue : self._getOptionValue(name))), 
    "boolean" == typeof self._getOptionValue(name) || void 0 === self._getOptionValue(name) ? null == val ? self._setOptionValue(name, !option.negate && (defaultValue || !0)) : self._setOptionValue(name, val) : null !== val && self._setOptionValue(name, !option.negate && val);
  })), this;
  ...
  return this._storeOptionsAsProperties = void 0 === value || value, this.options.length && console.error("Commander usage error: call storeOptionsAsProperties before adding options"), 
  this;
  ...
  var normalized = this.normalize(argv.slice(2)), parsed = this.parseOptions(normalized), args = this.args = parsed.args, result = this.parseArgs(this.args, parsed.unknown);
  "help" === args[0] && 1 === args.length && this.help(), "help" === args[0] ? (args[0] = args[1], 
  args[1] = this._helpLongFlag) : this._checkForMissingMandatoryOptions();
  var name = result.args[0], subCommand = null;
  return name && (subCommand = this.commands.find((function(command) {
    return command._name === name;
  }))), !subCommand && name && (subCommand = this.commands.find((function(command) {
    return command.alias() === name;
  }))) && (name = subCommand._name, args[0] = name), !subCommand && this.defaultExecutable && (name = this.defaultExecutable, 
  args.unshift(name), subCommand = this.commands.find((function(command) {
    return command._name === name;
  }))), this._execs.has(name) ? this.executeSubCommand(argv, args, parsed.unknown, subCommand ? subCommand._executableFile : void 0) : result;
}, Command.prototype.parseAsync = function(argv) {
  return this.parse(argv), Promise.all(this._actionResults);
}, Command.prototype.executeSubCommand = function(argv, args, unknown, executableFile) {
  (args = args.concat(unknown)).length || this.help();
  var baseDir, isExplicitJS = !1, pm = argv[1], bin = basename(pm, path.extname(pm)) + "-" + args[0];
  ...
  var proc, localBin = path.join(baseDir, bin);
  exists(localBin + ".js") ? (bin = localBin + ".js", isExplicitJS = !0) : exists(localBin + ".ts") ? (bin = localBin + ".ts", 
  isExplicitJS = !0) : exists(localBin + ".mjs") ? (bin = localBin + ".mjs", isExplicitJS = !0) : exists(localBin) && (bin = localBin), 
  ...
  exitCallback ? proc.on("close", (() => {
    ...
    if ("ENOENT" === err.code ? console.error("error: %s(1) does not exist, try --help", bin) : "EACCES" === err.code && console.error("error: %s(1) not executable. try chmod or run with root", bin), 
    exitCallback) {
  ...
  for (var arg, lastOpt, index, short, opt, ret = [], i = 0, len = args.length; i < len; ++i) {
    ...
    lastOpt && lastOpt.required ? ret.push(arg) : arg.length > 2 && "-" === arg[0] && "-" !== arg[1] ? (short = arg.slice(0, 2), 
    (opt = this.optionFor(short)) && (opt.required || opt.optional) ? (ret.push(short), 
    ret.push(arg.slice(2))) : arg.slice(1).split("").forEach((function(c) {
      ret.push("-" + c);
  ...
  var name;
  return args.length ? (name = args[0], this.listeners("command:" + name).length ? this.emit("command:" + args.shift(), args, unknown) : this.emit("command:*", args, unknown)) : (outputHelpIfRequested(this, unknown), 
  unknown.length > 0 && !this.defaultExecutable && this.unknownOption(unknown[0]), 
  0 === this.commands.length && 0 === this._args.filter((function(a) {
    return a.required;
  ...
  for (var cmd = this; cmd; cmd = cmd.parent) cmd.options.forEach((anOption => {
  ...
  i + 1 < argv.length && ("-" !== argv[i + 1][0] || "-" === argv[i + 1]) && unknownOptions.push(argv[++i])) : args.push(arg); else literal = !0;
  return {
    args: args,
  ...
  console.error(message), this._exit(1, "commander.variadicArgNotLast", message);
}, Command.prototype.version = function(str, flags, description) {
  if (0 === arguments.length) return this._version;
  this._version = str;
  var versionOption = new Option(flags = flags || "-V, --version", description = description || "output the version number");
  this._versionOptionName = versionOption.long.substr(2) || "version", this.options.push(versionOption);
  var self = this;
  return this.on("option:" + this._versionOptionName, (function() {
  ...
  var width = this.padWidth(), descriptionWidth = (process.stdout.columns || 80) - width - 4;
  ...
  var commands = this.prepareCommands(), width = this.padWidth(), descriptionWidth = (process.stdout.columns || 80) - width - 4;
  ...
  if (this._description) {
    ...
    if (argsDescription && this._args.length) {
      var width = this.padWidth(), descriptionWidth = (process.stdout.columns || 80) - width - 5;
  ...
  for (var parentCmdNames = "", parentCmd = this.parent; parentCmd; parentCmd = parentCmd.parent) parentCmdNames = parentCmd.name() + " " + parentCmdNames;
  var usage = [ "Usage: " + parentCmdNames + cmdName + " " + this.usage(), "" ], cmds = [], commandHelp = this.commandHelp();
  ...
  return splitFlags.length > 1 && (this._helpShortFlag = splitFlags.shift()), this._helpLongFlag = splitFlags.shift(), 
  this;
